...
async def list_categories(session: Annotated[AsyncSession, Depends(_session)]):
    rows = (await session.execute(select(Category).where(Category.is_active.is_(True)).order_by(Category.name.asc()))).scalars().all()
    existing_names = {c.name for c in rows}
    product_categories = (
    ...
    categories = [{"_id": c.id, "name": c.name, "slug": c.slug, "isActive": c.is_active} for c in rows] + synthetic
...
async def update_category(
    ...
    name = payload.name.strip()
    slug = _slugify(name)
    existing = await session.execute(select(Category).where((Category.name == name) | (Category.slug == slug), Category.id != category_id))
